    ...
    def load_file
       ...
       if !File.exists? file_to_load
           ...
           return
            ...
            load_students(filename) do |file_to_close|            #Code block to close the file.
                file_to_close.close
                puts "This is in the block. File #{file_to_close} has been closed now."
            end    
    end
    
    def load_students (filename = "students.csv")   #,&close_file)       #If "try_load_students" does not supply the file, then the default is used.   
       ...
       #file.close
       yield file             #yields to the block that is passed in every "time load_students" is called. This block closes the file.
       #close_file.call
    ...
    def try_load_students
        ...
        if filename.nil?
            load_students("students.csv")  do |file_to_close|            #Code block to close the file.
                file_to_close.close
                puts "This is in the block. File #{file_to_close} has been closed now."
            end    
            return
        ...
        if File.exists?(filename)
            load_students(filename) do |file_to_close| #If the file exists it is passed to "load_students"
